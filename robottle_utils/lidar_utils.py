...
def check_obstacle_ahead(distances, angles, save_index = None, length_to_check = 250, half_robot_width = 180, threshold_low = 12, threshold_high = 100):
    """Given LIDAR data, returns 'True' if there is an obstacle in front of the robot that matches the thresholding criterion
    ...
    PARAMETERS
    ----------
    distances, angles: Lidar output
    length_to_check: length of the rectangle to check [mm]
    half_robot_width (int)
    threshold_low (int): minimum number of rays that must be inside the rectangle
    threshold_high (int): maximm number of rays that must be inside the rectangle

    USES
    ----
    - detect a bottle: threshold_low = 1, threshold_high = 8
    - detect only big obstacles: threshold_low = 12 (DEFAULT), threshold_high = 1000 (DEFAULT)
